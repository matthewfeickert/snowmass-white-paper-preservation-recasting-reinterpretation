...
To help with clarity of discussion, we summarize key terms used in the paper in~\Cref{fig:glossary}.
...
% 
% \TODO{Preservation and sharing beyond a single experiment can be a $\Delta$ of effort on top of an experiment's core effort that becomes hard to resource and which may not be well recognised. Appropriate resources and career structures to support such work, which benefit the whole theory/phenomenology/experiment community, need to be planned and provided.}
...
\TODO{Giordon: Reintepretation/recasting should be basically the same thing? Not sure how to refine it separately. Whatever you do here, also fix it in the glossary.}
...
These might include summary plots and tables, histograms of kinematic distributions, fiducial cross sections, cross section limits, simplified model results, correlation information, analysis statistical workspace binaries or full statistical models, etc.
    ...
    \item[1.7:] \textbf{\Gls{recasting}}: Kinematic reinterpretation (kinematic distributions change....) %% to be refined
\end{description}
\end{tcolorbox}
...
Indeed there are very few examples of analyses where communicating machine-learned models has been tried. Concretely, the \gls{CMS} all-hadronic search for supersymmetry~\cite{CMS:2017qxu} published a simplified version of their top-quark tagger, which is based on a Random Forest decision tree~\cite{cms:toptagger}, and the \gls{ATLAS} 0-lepton gluino/squark search~\cite{ATLAS:2020syg,Uno:2763449} published the \gls{BDT} weights of their event selection as \texttt{XML} files on HEPData~\cite{hepdata.95664.v2/r8}.  
...
Further development along these lines is highly needed. Successful sharing of an \gls{ML} model requires not only sharing the model itself (including architectures, weights, and complete specification of software dependencies) but also the detailed specification of the input data. See also the corresponding discussion in Ref.~\cite{snowmass:MLevts} in this context.
...
% Giordon: Checked and tweaked a bit to fit the rest of Section 3 better
...
% Giordon: This is the point here -- big steps need to be taken to really push the community towards harmonization. Small steps only work when you've agreed on some set of standards.. but we haven't gotten there yet... 
...
The same platforms used for data product preservation (see~\Cref{data-products}) can also be used by the theory community to preserve the reinterpretation results and data.
...
\begin{appendices}
    ...
    One should note however that \gls{HEP} analyses are not always fully domain-specific and often need to execute custom logic in addition to predefined behaviors.
    ...
    \gls{ADL}, the only framework-independent example above, follows the former approach, and relies on an external database of injectable functions for each target framework.
\end{appendices}
