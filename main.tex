...
% \footnote{As should be the case in the paper publication, but rarely is given to the level of detail required for full reproduction.} % for space
...
% \EDITOR{Matthew: These next two paragraphs seem to be focusing on listing different examples of \glspl{DSL}.
...
%\COMMENT{SK: I want this in. It is relevant and would not be fair to take it out. Besides, there are lot of other technical references elsewhere, like yoda stuff, that are kept.}
%\COMMENT{GS: A format like YODA or JSON is different from a tool like FuncADL. One is a project that needs to be cited, and the other is very general.}


% On the embedded \gls{DSL} side, an early example is the \texttt{F.A.S.T.} framework~\cite{FAST} which incorporates a \texttt{YAML} \gls{DSL}.
% More recent developments include \texttt{NAIL} (Natural Analysis Implementation Language)~\cite{NAIL}, bamboo~\cite{David:2021ohq}, and FuncADL~\cite{Proffitt:2021wfh}, all based on  Python.
% The following is an attempt to get things rendered out okay so subsequent recommendation sections to not split other sections up. If the phrasing is awkward, just revert to the 2 sentences above.
On the embedded \gls{DSL} side, an early example is the \texttt{F.A.S.T.} framework~\cite{FAST}, used in CMS, \gls{LZ}, and \gls{DUNE} analyses, incorporates a \texttt{YAML} \gls{DSL}.
...
%There also exists the toy language PartiQL designed to demonstrate HEP-specific features requiring a radical departure from general purpose languages~\cite{PartiQL}. 
...
% \TODO{Get formatting to look okay for Data Product Preservation Recommendations not bleeding into the reinterpretation includes}
%-------------------------------------------------------------
...
%(a review of existing tools and approaches is given in Section~III of Ref.~\cite{LHCReinterpretationForum:2020xtr}). 
...
If adopted rigorously, our guidelines will allow for new scientific results, including testing of new models, for decades to come through reinterpretation %and recasting 
