...
In the following, we distinguish between preserving collision and simulated data used as input to physics analyses, described in Section~\ref{data-preservation}, and preserving workflows and data products connected to specific analyses, covered in Sections~\ref{analysis-preservation} and~\ref{data-products}, respectively. Section~\ref{reinterpret} addresses using existing data products in reinterpreting new physics scenarios.
...
\begin{description}
   \item[2.1:] Agree on data preservation and public data releases as a common goal to maximize their scientific outcome and allocate resources and responsibilities to achieve this goal in the experiment organization. %\ALERT{Kyle: incorporate budgeting/resources and experiment organization. Incorporate into operations and facilities. Kati: see updated 2.1 & 2.3  }
...
\TODO{Turing machines objection: HEP analyses are not fully domain-specific and often need to execute custom logic in addition to predefined behaviors. To cope, either we extend the DSL to incorporate new known behaviors providing an ``escape'' mechanism to inject general-purpose code (compromising its framework independence) or be combined with pre-and post-processing code to cover gaps in the DSL capabilities.}
...
\TODO{SK: to my mind, this is taken care of in the previous sections so that section 3.4 can be removed.}
...
\hrulex
The UTK group has been working towards implementing heavy-ion analyses in \rivet since Summer 2018. 
This proposal works towards an overarching goal of the UTK group: to make systematic comparisons between data and models, particularly incorporating jets' measurements.
This work in \rivet has incorporated most of the undergraduates listed in [table omitted] as summer students, students doing research for credit, and students in a Course-based Undergraduate Research Experience (CURE). CUREs were pioneered in biology and have increased graduation rates in STEM fields~\cite{Rodenbusch20062016}. The outreach also provides an opportunity for a more diverse group of students to participate in research. Approximately one-third of all UTK students graduating with bachelor's degrees in physics since Summer 2019 has worked on \rivet with our group, including about 40\% of all women and about half of all non-white students. This work has led to the implementation of approximately 20~analyses in \rivet.  
\hrulex
...
I am going to try to get a process for approval in PHENIX but was told I need to get something approval ready - I've been close for months, but the pandemic hit hard.
...
\TODO{Andy to rewrite this into a concise form giving the HI/EIC status and potential aligned with the Snowmass priorities.}
...
\COMMENT{SK: The above is a good start, but we need to go further than that. A unified statistical approach is nice, but what's needed are the actual statistical models to use to this end. pyhf can do the job when the analysis is based on Histfactory, but CMS uses a different approach; un-binned results need yet a different treatment, etc. So it is not that simple.}

\COMMENT{AL: The statistical models should be provided (in some format) by the experimental collaborations, no? I mentioned pyhf or a "similar package", since pyhf would have to be extended to deal with other formats or a new package would have to be developed.}
